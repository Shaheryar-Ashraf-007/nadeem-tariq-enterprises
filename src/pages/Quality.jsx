...
const compliance = [
  [
    "Kids Garment",
    "Flame-retardancy compliance, azo-dye-free certification, tag-free construction for under-3 sizing.",
  ],
  [
    "Pet Supplies",
    "Non-toxic material certification for feeding and chew-adjacent products, EU REACH-aligned material sourcing.",
  ],
  [
    "Offset Printing Ink",
    "Low-migration formulations available for food-adjacent packaging, batch-level SDS documentation.",
  ],
  [
    "Mobile Parts & Accessories",
    "Cell-certified battery sourcing, RoHS-aligned component sourcing, model-specific OEM spec matching.",
  ],
];
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center px-6 text-white">
          ...
          <p className=" font-body tracking-wide mt-4 text-center max-w-2xl sm:text-xl md:text-2xl text-gray-200">
            ...
            compliance, and continuous improvement across all our business
            divisions.
          </p>
        </div>
      </div>
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10 bg-green-600 text-white">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {pillars.map((p) => (
            <div
              key={p.title}
              className="rounded-md border border-paper-line bg-paper p-6 dark:border-ink-line dark:bg-ink"
            >
              <p.icon size={48} style={{ color: "var(--color-brass)" }} />
              <h3 className="mt-4 lg:text-[15px] sm:text-2xl font-body tracking-wide font-semibold">
                {p.title}
              </h3>
              <p className="mt-2 lg:text-sm sm:text-lg font-body tracking-wide text-ink/65 dark:text-paper/65">
                {p.text}
              </p>
            </div>
          ))}
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      ...
                      style={{
                        backgroundColor: `var(--color-${div.color}-soft)`,
                      }}
                    >
                      <Icon size={48} className="text-green-600" />
                    </span>
                    <span className="lg:text-md sm:text-xl font-body tracking-wide font-semibold text-green-600">
                      {name}
                    </span>
                  </div>
                  <p className="lg:text-sm sm:text-lg font-body tracking-wide leading-relaxed text-ink/70 dark:text-paper/70">
                    {desc}
                  </p>
                </div>
              );
